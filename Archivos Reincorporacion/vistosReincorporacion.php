...
<html lang="es">
  <head>
    <title>Vistos reincorporación</title>
  ...
  <body>
    <div class="container">
        ...
        <div class="row">
            <div class="col-xs-12">
                <h4 class="well well-sm text-center"><b>VISTOS REINCORPORACIÓN</b></h4>
            </div>
        </div>
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <input type="hidden" name="vrei_id" id="vrei_id" value="" />

                        <div class="form-group">
                            <label for="vrei_descripcion" class="control-label col-md-2 col-md-offset-1">Descripci&oacute;n:</label>
                            <div class="col-md-9">
                                <input type="text" name="vrei_descripcion" id="vrei_descripcion" class="form-control input-sm" required minlength="10" maxlength="255" placeholder="Descripci&oacute;n" title="Descripci&oacute;n"/>
                            </div>
                        </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <input type="hidden" name="vrei_id" id="delete_vrei_id" value="" />
                      </div>
                      <div class="modal-footer">
                        <button type="button" class="btn btn-default btn-cancelar" data-dismiss="modal">Cancelar</button>
    ...
    var listar = function(){
        var table = $("#dtVistos").DataTable({
            ...
            "ajax":{
                method: "GET",
                url: "crudVistosReincorporacion.php?opcion=leer"
            },
            "columnDefs":[
                {
                 targets: [0],
        ...
        $("#formCrearEditar").on("submit", function(e){
            ...
            form.validate();
             if (form.valid()==true){
                 $.ajax({
                     method: "POST",
                     url: "crudVistosReincorporacion.php",
                     data: formData
                 }).done(function( resp ){
                     limpiar_form(opcion);
                     mostrar_msj( resp );
    ...
    $('#modalCrearEditar').on('shown.bs.modal', function(){
        if( ! $("#vrei_descripcion").prop("readonly") ){
            $('#vrei_descripcion').focus();
        }else{
            $("#vrei_descripcion").focus();
        }
    });
    ...
    var obtener_datos = function(tbody, table){
        $(tbody).on("click", "button.editar", function(){
            ...
            $("#vrei_id").val( data.vrei_id );
            $("#vrei_descripcion").val( data.vrei_descripcion);
        });
    }
    ...
    var obtener_id = function(tbody, table){
        $(tbody).on("click", "button.eliminar", function(){
            var data = table.row( $(this).parents("tr") ).data();
            var vrei_id = $("#delete_vrei_id").val( data.vrei_id ),
            vrei_descripcion = $("#registro").text( data.vrei_descripcion );
        });
    }
