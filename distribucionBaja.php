...
<html lang="es">
  <head>
    <title>Distribuci&oacute;n Baja</title>
  ...
  <body>
    <div class="container">      
        ...
        <div class="row">
            <div class="col-xs-12">
                <h4 class="well well-sm text-center"><b>DISTRIBUCION BAJAS</b></h4>
            </div>
        </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <input type="hidden" name="dbaj_id" id="dbaj_id" value="" />
                        
                        <div class="form-group">
                            <label for="dalt_descripcion" class="control-label col-md-2 col-md-offset-1">Descripci&oacute;n:</label>
                            <div class="col-md-9">
                                <input type="text" name="dbaj_descripcion" id="dbaj_descripcion" class="form-control input-sm" required minlength="10" maxlength="255" placeholder="Descripci&oacute;n" title="Descripci&oacute;n"/>
                            </div>                
                        </div> 
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">                    
                        ...
                        <input type="hidden" name="dbaj_id" id="delete_dbaj_id" value="" />
                      </div>
                      <div class="modal-footer">
                        <button type="button" class="btn btn-default btn-cancelar" data-dismiss="modal">Cancelar</button>
    ...
    var listar = function(){        
        var table = $("#dtDistrib").DataTable({            
            ...
            "ajax":{
                method: "GET",
                url: "crudDistBaja.php?opcion=leer"
            },
            "columnDefs":[
                 {
                     targets: [0],
    ...
    $('#modalCrearEditar').on('shown.bs.modal', function(){
        if( ! $("#dbaj_descripcion").prop("readonly") ){
            $('#dbaj_descripcion').focus();
        }else{
            $("#dbaj_descripcion").focus();
        }
    });
    ...
    var obtener_datos = function(tbody, table){
        $(tbody).on("click", "button.editar", function(){            
            ...
            $("#dbaj_id").val( data.dbaj_id );
            $("#dbaj_descripcion").val( data.dbaj_descripcion);              
        });        
    }
    ...
    var obtener_id = function(tbody, table){
        $(tbody).on("click", "button.eliminar", function(){
            var data = table.row( $(this).parents("tr") ).data();
            var dbaj_id = $("#delete_dbaj_id").val( data.dbaj_id ),
            dbaj_descripcion = $("#registro").text( data.dbaj_descripcion );
        });        
    }
