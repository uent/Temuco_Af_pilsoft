...
<html lang="es">
  ...
  <body>
    <div class="container">
        ...
        <div class="row">
            <div class="col-xs-12">
                <h4 class="well well-sm text-center"><b>DISTRIBUCION REINCORPORACIÓN</b></h4>
            </div>
        </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <input type="hidden" name="drei_id" id="drei_id" value="" />

                        <div class="form-group">
                            <label for="drei_descripcion" class="control-label col-md-2 col-md-offset-1">Descripci&oacute;n:</label>
                            <div class="col-md-9">
                                <input type="text" name="drei_descripcion" id="drei_descripcion" class="form-control input-sm" required minlength="10" maxlength="255" placeholder="Descripci&oacute;n" title="Descripci&oacute;n"/>
                            </div>
                        </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <input type="hidden" name="drei_id" id="delete_drei_id" value="" />
                      </div>
                      <div class="modal-footer">
                        <button type="button" class="btn btn-default btn-cancelar" data-dismiss="modal">Cancelar</button>
    ...
    var listar = function(){
        var table = $("#dtDistrib").DataTable({
            ...
            "ajax":{
                method: "GET",
                url: "crudDistReincorporacion.php?opcion=leer"
            },
            "columnDefs":[
                 {
                     targets: [0],
        ...
        $("#formCrearEditar").on("submit", function(e){
            ...
            form.validate();
             if (form.valid()==true){
                 $.ajax({
                     method: "POST",
                     url: "crudDistReincorporacion.php",
                     data: formData
                 }).done(function( resp ){
                     limpiar_form(opcion);
                     mostrar_msj( resp );
    ...
    $('#modalCrearEditar').on('shown.bs.modal', function(){
        if( ! $("#drei_descripcion").prop("readonly") ){
            $('#drei_descripcion').focus();
        }else{
            $("#drei_descripcion").focus();
        }
    });
    ...
    var obtener_datos = function(tbody, table){
        $(tbody).on("click", "button.editar", function(){
            ...
            $("#drei_id").val( data.drei_id );
            $("#drei_descripcion").val( data.drei_descripcion );
        });
    }
    ...
    var obtener_id = function(tbody, table){
        $(tbody).on("click", "button.eliminar", function(){
            var data = table.row( $(this).parents("tr") ).data();
            var drei_id = $("#delete_drei_id").val( data.drei_id ),
            drei_descripcion = $("#registro").text( data.drei_descripcion );
        });
    }
