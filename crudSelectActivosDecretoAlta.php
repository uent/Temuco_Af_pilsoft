 ...
 *                     return '<input type="checkbox" name="act_id[]" value="'.$d.'"/>';
 *                }
 *         ),
 */ 
    ...
    $extraWhere = "act_codEstado='V' AND act_decretoAlta=0 AND act_codigo=act_idLote";
    $groupBy = ''; // "`u`.`office`";
...
function leerData($conexion)
{
                                ...
                                JOIN ubicaciones UB
                                    ON AF.ubi_id=UB.ubi_id
                                        JOIN responsables RE
                                            ON UB.resp_id=RE.resp_id
                                                WHERE act_codEstado='V' AND act_decretoAlta=0 
    ...
    while( $data = mysqli_fetch_assoc($result) ){
        ...
        //$data['act_valorAdquisicion']=( $data['act_valorAdquisicion']!=0) ? number_format($data['act_valorAdquisicion'], 0, ',', '') : 0;
